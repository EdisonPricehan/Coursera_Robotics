...
while true
    ...
    neighbor = zeros(1,4);
    if (j-1 > 0)
        neighbor(1) = sub2ind(size(map), i, j-1);
    end
    
    if (j+1 <= ncols)
        neighbor(2) = sub2ind(size(map), i, j+1);
    end
    
    if (i-1 > 0)
        neighbor(3) = sub2ind(size(map), i-1, j);
    end
    
    if (i+1 <= nrows)
        neighbor(4) = sub2ind(size(map), i+1, j); 
    end
    
    if (j == 1)
        neighbor(1) = sub2ind(size(map), i, ncols);
    end
    
    if (j == ncols)
        neighbor(2) = sub2ind(size(map), i, 1);
    end
    
    if (i == 1)
        neighbor(3) = sub2ind(size(map), nrows, j);
    end
    
    if (i == nrows)
        neighbor(4) = sub2ind(size(map), 1, j);
    end
    
    for n = 1:length(neighbor)
        [a,b] = ind2sub(size(map),neighbor(n));
        update(a, b, min_dist+1, current);
    end
    % *******************************************************************
end
